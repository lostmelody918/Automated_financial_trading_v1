...
def plot_dynamic_dashboard(df, subplot_features_list, df_trades=None):
    ...
    # 確保開啟網格，依賴 hovermode='x' 自動產生貫穿全部圖表的垂直游標
    fig.update_xaxes(
...
if engine is None:
    ...
    def update_dashboard():
        try:
            ...
            # 1. 載入舊版 CSV 歷史紀錄
            base_path_csv = "data_learn"
            if not os.path.exists(base_path_csv):
                base_path_csv = "F:/Gemini_CLI_Application/finance_v2/data_learn"
            
            report_files_csv = glob.glob(os.path.join(base_path_csv, "daily_trade_report_*.csv"))
            for f in report_files_csv:
                try: dfs.append(pd.read_csv(f))
                except Exception: pass

            # 2. 載入新版 JSONL 歷史紀錄 (包含今天的覆寫檔)
            base_path_jsonl = "reports_output"
            if not os.path.exists(base_path_jsonl):
                base_path_jsonl = "F:/Gemini_CLI_Application/finance_v2/ai_short_term_day_trading/reports_output"
                
            report_files_jsonl = glob.glob(os.path.join(base_path_jsonl, "cli_trade_log_*.jsonl"))
            for f in report_files_jsonl:
                try: 
                    # 讀取 JSONL 檔案
