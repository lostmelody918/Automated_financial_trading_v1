...
# Path setup for simulator
base_dir = os.path.dirname(os.path.abspath(__file__)) # options_backtester
    ...
    if not df_intraday.empty:
        ...
        if trade_log:
            ...
            rename_cols = {
                ...
                'symbol': '合約代碼',
                'type': '方向',
                'strategy_label': '觸發策略 (Strategy)',
                'entry_price': '進場價',
                'exit_price': '出場價',
                'pnl': '損益 (NT$)'
            }
            
            core_cols = [c for c in rename_cols.keys() if c in df_trades.columns]
            df_trades_display = df_trades[core_cols].rename(columns=rename_cols)
            ...
            with st.expander("🔍 查看詳細進場特徵參數 (Entry Features)"):
                feature_cols = [c for c in df_trades.columns if c not in rename_cols.keys()]
                if feature_cols:
                    # Select entry time and strategy label for reference, then all feature columns
                    disp_feats = ['entry_time']
                    if 'strategy_label' in df_trades.columns:
                        disp_feats.append('strategy_label')
                    disp_feats.extend(feature_cols)
                    df_feats = df_trades[disp_feats]
        ...
        if all_options and not df_ticks.empty:
            ...
            with col_t1:
                entry_str = st.text_input("設定進場時間 (時:分:秒)", value="09:00:00")
                try:
                    manual_entry = datetime.strptime(entry_str, "%H:%M:%S").time()
                except ValueError:
                    st.error("格式錯誤，請使用 HH:MM:SS")
                    manual_entry = time(9, 0, 0)
            with col_t2:
                exit_str = st.text_input("設定出場時間 (時:分:秒)", value="09:30:00")
                try:
                    manual_exit = datetime.strptime(exit_str, "%H:%M:%S").time()
                except ValueError:
                    st.error("格式錯誤，請使用 HH:MM:SS")
                    manual_exit = time(9, 30, 0)
            ...
            if manual_entry >= manual_exit:
                ...
                if df_sym_ticks.empty:
                    ...
                    if entry_mask.empty or exit_mask.empty:
                        ...
                        # 畫出合約走勢與模擬進出場 (降採樣或使用 WebGL 以優化效能)
                        # 如果 tick 數量太大，進行簡單降採樣 (每隔 N 筆取一筆)
                        max_points = 5000
                        if len(df_sym_ticks) > max_points:
                            step = len(df_sym_ticks) // max_points
                            df_plot_ticks = df_sym_ticks.iloc[::step]
                        else:
                            df_plot_ticks = df_sym_ticks

                        fig_opt = go.Figure()
                        fig_opt.add_trace(go.Scattergl(
                            x=df_plot_ticks['time'],
                            y=df_plot_ticks['price'],
                            mode='lines',
