...
class PostMarketExporter:
    ...
    """
    def __init__(self, export_dir=r"F:\Gemini_CLI_Application\finance_v2\ai_short_term_day_trading\reports_output"):
            ...
            try:
                ...
                for t in trade_log:
                    ...
                    if pd.notna(entry_time):
                        entry_time = entry_time.replace(tzinfo=None) # 時區防呆
                    if pd.notna(exit_time):
                        exit_time = exit_time.replace(tzinfo=None) # 時區防呆
    ...
    def execute_export(self, df_market, dict_options_history, trade_log):
        """主入口"""
        # 改為以天為單位的單一檔案，每次觸發時複寫 (覆蓋)，避免資料遺失且只有一個檔
        date_str = datetime.now().strftime('%Y%m%d')
        print(f"🚀 開始匯出資料... (覆寫模式，批次 {date_str})")
