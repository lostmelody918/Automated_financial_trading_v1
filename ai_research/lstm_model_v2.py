...
def train_and_predict(stock_id="2330", market="TW"):
    ...
    with torch.no_grad():
        ...
        print("\n🔍 --- 模型輸出診斷 ---")
        print(f"預測收益率 最大值: {preds_ret.max():.6f}")
        print(f"預測收益率 最小值: {preds_ret.min():.6f}")
        print(f"實際收益率 最大值: {actual_ret.max():.6f}")

        # 🎯 1. 將數值預測轉換為「交易訊號」 (動態閾值)
        dynamic_threshold = np.percentile(np.abs(preds_ret), 70)
        print(f"動態交易閾值設定為: {dynamic_threshold:.6f}")

        signals = np.zeros(len(preds_ret))

        # 防呆機制：若全為 0，則不進行交易
        if dynamic_threshold > 0:
            # 🚀 關鍵：加入 >= 確保訊號能被觸發
            signals[preds_ret >= dynamic_threshold] = 1
            signals[preds_ret <= -dynamic_threshold] = -1
        ...
        if np.sum(active_days) > 0:
            correct_dir = ((signals[active_days] > 0) == (actual_ret[active_days] > 0)).sum()
            dir_accuracy = (correct_dir / np.sum(active_days)) * 100
        else:
            dir_accuracy = 0.0

        print(f"\n📈 --- 測試集分析報告 ---")
        print(f"總交易天數: {len(actual_ret)} 天")
        print(f"觸發交易訊號天數: {np.sum(active_days)} 天")
        print(f"有效方向準確率: {dir_accuracy:.2f}%")
    ...
    plt.plot(cumulative_strategy, label='Attention-LSTM 策略報酬', color='#d93025', lw=2.5)

    plot_days = min(100, len(actual_ret))
    recent_signals = signals[-plot_days:]
    recent_strategy = cumulative_strategy[-plot_days:]

    buy_idx = np.where(recent_signals == 1)[0] + (len(actual_ret) - plot_days)
    sell_idx = np.where(recent_signals == -1)[0] + (len(actual_ret) - plot_days)

    plt.scatter(buy_idx, cumulative_strategy[buy_idx], marker='^', color='#27ae60', s=100, label='AI 買進訊號', zorder=5)
    plt.scatter(sell_idx, cumulative_strategy[sell_idx], marker='v', color='#8e44ad', s=100, label='AI 放空訊號', zorder=5)

    plt.title(f'{stock_id} 自動化交易策略回測：資金曲線與買賣訊號', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    save_path = f"ai_research/{stock_id}_equity_curve.png"
