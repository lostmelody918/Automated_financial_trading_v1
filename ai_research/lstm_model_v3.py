...
def train_and_predict(stock_id="2330", market="TW"):
    ...
    with torch.no_grad():
        ...
        sell_threshold = np.percentile(neg_preds, 50) if len(neg_preds) > 0 else -0.001
        ...
        for i in range(len(smoothed_preds)):
            ...
            # 🚀 定義「強勢技術突破」：多頭趨勢 + RSI強勢區(>60) + MACD正向發散
            ...
            # 情況 B：AI 看空
                ...
                if not is_bull_trend and is_foreign_selling:
                    signals[i] = -1.0 # 雙重確認，果斷放空
                elif is_strong_breakout:
                    # 🛡️ 突破覆寫：AI 雖然害怕想賣，但技術面正在噴發，拒絕下車！
                    signals[i] = 1.0
                else:
                    signals[i] = 0.0  # 正常避險

            # 情況 C：AI 覺得是雜訊 (盤整或趨勢延續)
            else:
                if is_strong_breakout:
                    signals[i] = 1.0  # 🛡️ 突破覆寫：技術面強勢，滿倉讓利潤奔跑
                elif is_bull_trend:
                    signals[i] = 0.5  # 普通牛市：半倉跟隨 (降低曝險)
                else:
                    signals[i] = 0.0  # 熊市盤整：空手觀望
        ...
        # 重新計算有效勝率
        active_ai_days = (signals != 0) & (signals != 0.5) # 僅計算做出明確決策(滿倉/放空)的日子
        ...
        print(f"滿倉突破: {days_full} 天 | 半倉跟隨: {days_half} 天 | 確認放空: {days_short} 天 | 避險空手: {days_empty} 天")
    ...
    plt.plot(cumulative_strategy, label='Attention-LSTM (突破+籌碼) 策略報酬', color='#d93025', lw=2.5)
    ...
    plt.scatter(short_idx, cumulative_strategy[short_idx], marker='v', color='#8e44ad', s=150, label='籌碼確認放空', zorder=5)
    plt.scatter(exit_idx, cumulative_strategy[exit_idx], marker='x', color='#7f8c8d', s=100, label='清倉避險', zorder=5)

    plt.title(f'{stock_id} 自動化交易策略：強勢突破抱緊與多空防護', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    save_path = f"ai_research/{stock_id}_equity_curve_breakout.png"
