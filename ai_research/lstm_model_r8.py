...
class StockDataProcessor:
    def __init__(self, window_size=15):
        ...
        # 🚀 確保這裡包含 Slope_5D 與 Breakout_20D
        self.feature_cols = [
            'Log_Ret', 'Volume_Ratio', 'RSI', 'MACD', 'Bias20', 'Slope_5D', 'Foreign_Trend',
            'Momentum_3', 'Momentum_5', 'Breakout_20D', 'Trend_Regime',
            'Nasdaq_Ret', 'SOX_Ret', 'VIX_Ret', 'Volatility', 'ADR_Premium'
        ]
    ...
    def add_indicators(self, df, stock_id, is_us=False):
        ...
        # --- 🚀 終極標籤工程：未來 5 天區間極值 (Triple Barrier Approximation) ---
        # 尋找未來 1~5 天內的「最高價」與「最低價」
        ...
        if not is_us:
            try:
                ...
                adr_mapping = {"2330": {"symbol": "TSM", "ratio": 5}, "2303": {"symbol": "UMC", "ratio": 5}}
    ...
    def _create_windows(self, features, targets_max, targets_min, raw_ret_test=None):
        ...
        for i in range(len(features) - self.window_size):
            ...
            # 🚀 屏障判斷邏輯：
            # 如果未來 5 天能漲超過 2.5%，且即使回檔也不會跌破 1.5%，這就是無腦抱緊的「強勢續漲波段」！
            if f_max > 0.025 and f_min > -0.015:
                y.append(2) # 續漲波段
            # 如果未來 5 天會跌破 2.5%，且連 1.5% 的反彈都沒有，這是「瀑布式續跌」！
            elif f_min < -0.025 and f_max < 0.015:
                y.append(0) # 續跌波段
            else:
                y.append(1) # 震盪洗盤，空手觀望
...
def train_and_predict(stock_id="2330", market="TW"):
    ...
    print(f"🚀 【高斜率波段捕捉型】 啟動 (標的: {stock_id})")
    ...
    with torch.no_grad():
        ...
        print(f"續漲抱緊: {np.sum(signals == 1)} 天 | 續跌放空: {np.sum(signals == -1)} 天 | 洗盤觀望: {np.sum(signals == 0)} 天")
    ...
    plt.plot(cumulative_strategy, label='AI 屏障波段報酬', color='#d93025', lw=2.5)

    buy_idx = np.where(signals == 1.0)[0]
    short_idx = np.where(signals == -1.0)[0]

    plt.scatter(buy_idx, cumulative_strategy[buy_idx], marker='^', color='#27ae60', s=70, label='續漲抱緊', zorder=5)
    plt.scatter(short_idx, cumulative_strategy[short_idx], marker='v', color='#8e44ad', s=70, label='續跌放空', zorder=5)

    plt.title(f'{stock_id} 機器學習：三重屏障標籤與高斜率追蹤', fontsize=16, fontweight='bold')
    plt.xlabel('測試期天數')
    plt.ylabel('累積資金成長率 (%)')
    ...
    plt.title(f'{stock_id} 波段特徵重要性 (三重屏障標籤)', fontsize=14, fontweight='bold')
