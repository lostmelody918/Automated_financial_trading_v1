...
def train_and_predict(stock_id="2330", market="TW"):
    ...
    with torch.no_grad():
        ...
        print(f"📊 預測收益率 波動範圍: {preds_ret.min():.4f} ~ {preds_ret.max():.4f}")
        print(f"📊 實際收益率 波動範圍: {actual_ret.min():.4f} ~ {actual_ret.max():.4f}")

    # -----------------------------------------
    # 繪圖：預測收益率 vs 實際收益率 (不再畫絕對價格)
    # -----------------------------------------
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(18, 7))

    # 子圖 1：收益率折線圖 (取最近 100 天)
    plot_days = min(100, len(actual_ret))
    ax1.plot(actual_ret[-plot_days:], label='實際每日收益率 (Log Return)', color='#1a73e8', alpha=0.7)
    # 將預測收益率放大 5 倍，純粹為了在視覺上更容易看出它的起伏方向
    ax1.plot(preds_ret[-plot_days:] * 5, label='預測收益率 (放大5倍以便觀察方向)', color='#d93025', linestyle='--')
    ax1.axhline(0, color='black', linewidth=1, alpha=0.5)
    ax1.set_title(f'{stock_id} 最近 {plot_days} 天收益率預測趨勢', fontsize=14)
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    # 子圖 2：多空方向散佈圖 (Scatter Plot)
    # 這是檢驗模型有沒有預測能力的終極指標
    ax2.scatter(actual_ret, preds_ret, alpha=0.5, color='#8e44ad')
    ax2.axhline(0, color='red', linestyle='--', alpha=0.5, label='預測多空分界')
    ax2.axvline(0, color='blue', linestyle='--', alpha=0.5, label='實際多空分界')

    # 標示出四個象限
    ax2.text(actual_ret.max()*0.5, preds_ret.max()*0.5, '看多做對\n(True Positive)', fontsize=12, ha='center', color='green')
    ax2.text(actual_ret.min()*0.5, preds_ret.min()*0.5, '看空做對\n(True Negative)', fontsize=12, ha='center', color='green')
    ax2.text(actual_ret.min()*0.5, preds_ret.max()*0.5, '看多做錯\n(False Positive)', fontsize=12, ha='center', color='red')
    ax2.text(actual_ret.max()*0.5, preds_ret.min()*0.5, '看空做錯\n(False Negative)', fontsize=12, ha='center', color='red')

    ax2.set_xlabel('實際收益率', fontsize=12)
    ax2.set_ylabel('預測收益率', fontsize=12)
    ax2.set_title('多空決策象限圖', fontsize=14)
    ax2.legend()
    ax2.grid(True, alpha=0.3)

    plt.tight_layout()
    save_path = f"ai_research/{stock_id}_returns_analysis.png"
    plt.savefig(save_path)
    plt.show()
    print(f"✅ 收益率真相圖表已儲存至: {save_path}")
