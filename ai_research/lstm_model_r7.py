...
class StockDataProcessor:
    def __init__(self, window_size=15):
        ...
        # 🚀 關鍵修正：移除絕對數值的 MA20 與 Volume，換成穩態的 Bias20 與 Volume_Ratio
        self.feature_cols = [
            'Log_Ret', 'Volume_Ratio', 'RSI', 'MACD', 'Bias20', 'Foreign',
    ...
    def add_indicators(self, df, stock_id, is_us=False):
        ...
        # 1. 乖離率 (取代 MA20)：計算價格偏離月線的百分比
        df['Bias20'] = (df['Close'] - df['MA20']) / df['MA20']
        # 2. 爆量比率 (取代絕對成交量)：今日成交量除以近 20 日均量
        df['Volume_Ratio'] = df['Volume'] / df['Volume'].rolling(20, min_periods=1).mean()

        # 🚀 目標波段工程：預測未來 5 天 (一整週) 的波段漲跌
        df['Target_Ret'] = np.log(df['Close'].shift(-5) / df['Close'])
...
def train_and_predict(stock_id="2330", market="TW"):
    ...
    print(f"🚀 【波段趨勢追蹤型】 啟動 (標的: {stock_id})")
    ...
    with torch.no_grad():
        ...
        print(f"順勢做多: {np.sum(signals == 1)} 天 | 波段做空: {np.sum(signals == -1)} 天 | 觀望空手: {np.sum(signals == 0)} 天")
        print(f"AI 波段交易勝率: {strategy_win_rate:.2f}%")
    ...
    plt.plot(cumulative_strategy, label='AI 波段趨勢追蹤報酬', color='#d93025', lw=2.5)

    buy_idx = np.where(signals == 1.0)[0]
    short_idx = np.where(signals == -1.0)[0]

    plt.scatter(buy_idx, cumulative_strategy[buy_idx], marker='^', color='#27ae60', s=70, label='波段做多', zorder=5)
    plt.scatter(short_idx, cumulative_strategy[short_idx], marker='v', color='#8e44ad', s=70, label='波段做空', zorder=5)

    plt.title(f'{stock_id} 機器學習原生策略：長波段視野與特徵穩態化', fontsize=16, fontweight='bold')
    plt.xlabel('測試期天數')
    plt.ylabel('累積資金成長率 (%)')
    ...
    # 🔍 附錄：XAI 特徵重要性分析
    # ==========================================
    print("\n🕵️‍♂️ 啟動 XAI 特徵重要性分析 (觀察 T+5 波段驅動因子)...")
    ...
    plt.title(f'{stock_id} 波段預測特徵重要性 (T+5 Target)', fontsize=14, fontweight='bold')
