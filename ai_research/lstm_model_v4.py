...
def train_and_predict(stock_id="2330", market="TW"):
    ...
    with torch.no_grad():
        ...
        for i in range(len(smoothed_preds)):
            ...
            # 情況 B：AI 看空
                ...
                if not is_bull_trend and is_foreign_selling:
                    # 🛡️ 波段超跌防護：跌幅已深，拒絕追空，轉為空手準備
                    if is_oversold:
                        signals[i] = 0.0
                    else:
                        signals[i] = -1.0 # 高位起跌剛破線，果斷放空
        ...
        print(f"滿倉突破: {days_full} 天 | 半倉跟隨: {days_half} 天 | 高位放空: {days_short} 天 | 超跌/避險空手: {days_empty} 天")
    ...
    plt.plot(cumulative_strategy, label='Attention-LSTM (全防護) 策略報酬', color='#d93025', lw=2.5)
    ...
    plt.scatter(short_idx, cumulative_strategy[short_idx], marker='v', color='#8e44ad', s=150, label='高位破線放空', zorder=5)
    plt.scatter(exit_idx, cumulative_strategy[exit_idx], marker='x', color='#7f8c8d', s=100, label='超跌/清倉避險', zorder=5)

    plt.title(f'{stock_id} 自動化交易策略：高位做空與波段超跌保護', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    save_path = f"ai_research/{stock_id}_equity_curve_final.png"
